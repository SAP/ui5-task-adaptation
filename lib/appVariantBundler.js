...
module.exports = ({ workspace, options, taskUtil, cfUtil }) => {
	...
	/**
	 * Update sap.platform.cf auth scopes and sap.cloud service
	 *
	 * @param {App} baseApp base application
	 */
	function updateCloudPlatform(baseApp) {
		let sapCloudService = baseApp.manifestJson["sap.cloud"] && baseApp.manifestJson["sap.cloud"].service;
		let sapPlatformCf = baseApp.manifestJson["sap.platform.cf"];
		...
		if (options.configuration.sapCloudService) {
			if (baseApp.manifestJson["sap.cloud"] == null) {
				baseApp.manifestJson["sap.cloud"] = {};
			}
			baseApp.manifestJson["sap.cloud"].service = options.configuration.sapCloudService;
		} else {
	...
	async function getOpenUI5Path() {
		let openUi5Path = await getInstalledPath("@openui5", { local: true });
		if (fs.existsSync(openUi5Path)) {
			return openUi5Path;
		}
		openUi5Path = path.resolve(`${__dirname}/../../../@openui5`);
		if (fs.existsSync(openUi5Path)) {
			return openUi5Path;
		}
		log.error("@openui5 module is not installed");
	}

	async function readUI5Loader(openUi5Path) {
		return new Promise((resolve, reject) => {
			// @ts-ignore
			fs.readFile(`${openUi5Path}/sap.ui.core/src/ui5loader.js`, { encoding: "UTF-8" }, (error, data) => {
				if (error) {
					reject(error);
				}
				resolve(data);
	...
	async function configUI5Loader(ui5loaderFile, openUi5Path) {
		// @ts-ignore
		global.window = {};
		const ui5loaderFileFunction = new Function("window", "document", "XMLHttpRequest", ui5loaderFile);
		ui5loaderFileFunction(global, { baseURI: `file://` }, XMLHttpRequest);
		// @ts-ignore
		const ui5loader = global.sap && global.sap.ui && global.sap.ui.loader;
		// @ts-ignore
		global.define = global.sap.ui.define;
		// @ts-ignore
		global.define.amd = {};
		ui5loader.config({
			paths: {
				"sap/ui/fl": `file://${openUi5Path}/sap.ui.fl/src/sap/ui/fl`,
				"sap/base": `file://${openUi5Path}/sap.ui.core/src/sap/base`,
				"sap/ui/thirdparty/URI": `file://${openUi5Path}/sap.ui.core/src/sap/ui/thirdparty/URI`,
				"sap/ui/fl/Change": `file://${__dirname}/change`
			}
		});
	}

	async function initUi5Loader() {
		log.info("Initializing UI5 Loader");
		let openUi5Path = await getOpenUI5Path();
		const ui5loaderFile = await readUI5Loader(openUi5Path);
		await configUI5Loader(ui5loaderFile, openUi5Path);
	}
	...
	async function applyDescriptorChanges(baseApp, appVariant, module) {
		...
		return new Promise((resolve, reject) => {
			// @ts-ignore
			global.sap.ui.define([
				"sap/ui/fl/apply/_internal/changes/descriptor/Applier",
				"sap/ui/fl/apply/_internal/changes/descriptor/ApplyUtil",
				"sap/ui/fl/requireAsync",
				"sap/ui/fl/Change",
			], async (Applier, ApplyUtil, requireAsync, Change) => {
				const strategy = new BuildStrategy(requireAsync, ApplyUtil, module);
				const changes = appVariant.manifestJson.content.map((content) => new Change(content));
				baseApp.manifestJson = await Applier.applyChanges(baseApp.manifestJson, changes, strategy);
				resolve(baseApp.manifestJson);
			});
		});
	}
};