...
describe("Rollup", () => {
    ...
    it("should rollup if the bundle doesn't exist", async () => {
        ...
        const rollupStub = await prepareStubs(sandbox);
    ...
    it("should throw error if framework name is incorrect", async () => {
        await testUi5YamlValidation(sandbox, "ui5-incorrect-framework.yaml");
        await expect(RollupBuilder.run("./dist/bundle.js")).to.be.rejectedWith("ui5.yaml is not found or incorrect")
    });
    it("should throw error if framework name is incorrect", async () => {
        await testUi5YamlValidation(sandbox, "ui5-incorrect-version.yaml");
        await expect(RollupBuilder.run("./dist/bundle.js")).to.be.rejectedWith("ui5.yaml is not found or incorrect")
    });
});

async function testUi5YamlValidation(sandbox: sinon.SinonSandbox, testUi5Yaml: string) {
...
async function prepareStubs(sandbox: sinon.SinonSandbox) {
