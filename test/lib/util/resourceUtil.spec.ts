...
describe("ResourceUtil.readInProject", () => {
    it("reads a file from current project root", async () => {
        await withTempCwd(async (temp) => {
            ...
            expect(await ResourceUtil.readInProject(filename)).to.equal(content);
        });
    });

    it("throws custom error when not in project root (no ui5.yaml)", async () => {
        await withTempCwd(async () => {
            await expect(ResourceUtil.readInProject("missing.json"))
