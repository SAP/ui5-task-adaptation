    ...
    for (let i = 0; i < expectedFiles.length; i++) {
        ...
        const assertContents = ([currentContent, expectedContent]: string[]) => {
            if (expectedFile.endsWith("sap-ui-cachebuster-info.json")) {
                ...
                if (!expectedFile.endsWith(".properties")) {
                    expect(currentContent.replace(newLineRegexp, "\n"), relativeFile).to.eql(expectedContent.replace(newLineRegexp, "\n"));
                } else {
                    const expectedValues = getPropertyValues(expectedContent);
                    const actualValues = getPropertyValues(currentContent);
                    expect(actualValues, relativeFile).to.have.members(expectedValues);
                    expect(currentContent.split(newLineRegexp).sort(), relativeFile).to.have.members(expectedContent.split(newLineRegexp).sort());
                }
            }
        };
...
function getPropertyValues(expectedContent: string) {
    return expectedContent.split(newLineRegexp).map((line) => line.split("=")[1]).toSorted();
}
