...
import TraverseReferences from "../../../src/annotations/transformers/traverseReferences";
...
describe("TraverseReferences", () => {
    ...
    it("should add referenced annotations with different targets", async () => {
        ...
        sandbox.stub(serviceRequestor, "downloadAnnotation")
            // level 0
            .withArgs(PARENT_URL + "$metadata", PARENT_NAME, languages.find(l => l.isDefault))
            .resolves(TestUtil.getResourceXml(PARENT_FOLDER + "metadata-en.xml"))
            .withArgs(PARENT_URL + "$metadata", PARENT_NAME, languages.find(l => l.sap === "DE"))
        ...
        const expected = TestUtil.getResourceXml("annotations/v4/metadata-2-v4-expected/metadata.xml");
        expect((await files).get("annotations/annotation_mainService.xml")).to.eql(expected);
    });
    ...
    it("should not traverse metadata reference", async () => {
        const odata = metadataV4Xml(
            `<edmx:Reference Uri="../reference/to/child1">
    ...
    describe("should have the same result if", () => {
        [
            ...
            it(`join urls ${parentUrl.endsWith("/") ? "with" : "without"} trailing slash`, () => {
                expect(TraverseReferences.joinUrls("../../../sap/fco_/$metadata", parentUrl)).to.eql("/sap/opu/odata/sap/fco_/$metadata");
            });
        });
    });
